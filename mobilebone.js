...
(function (root, factory) {
	...
	**/
    ...
    if (window.navigator.userAgent.indexOf('Firefox') >= 0 && window.top !== window) {
	...
	**/
    Mobilebone.transition = function (pageInto, pageOut, back, options) {
        if (arguments.length == 0 || pageInto == pageOut)
            return;
        ...
        if (typeof preventOut == 'function')
            isPreventOut = preventOut.call(params_out.root, pageInto, pageOut, options);
        ...
        if (typeof preventInto == 'function')
            isPreventInto = preventInto.call(params_in.root, pageInto, pageOut, options);
        ...
        var fun_animationCall = function (page, data) {
            if (page.flagAniBind == true)
                return;
            // do callback when animation start/end
            [
                'animationstart',
                'animationend'
            ].forEach(function (animationkey, index) {
        ...
        if (pageOut != null && pageOut.classList) {
            ...
            if (isPreventOut != true) {
                ...
                if (typeof fallback == 'function')
                    fallback.call(params_out.root, pageInto, pageOut, options);
            }
        }
        if (pageInto != null && pageInto.classList) {
            // for title change
            var title = params_in.title, header = document.querySelector('h1'), first_page = document.querySelector('.' + this.classPage);
            // do title change	
            ...
            // do transition
            if (pageOut)
                pageInto.classList.add(params_in.form);
            // iOS bug 
            ...
            if (supportHistory && this.pushStateEnabled && options.history !== false && url_push    // hash should be different
...
&& url_push_replaced != location.hash) {
            ...
            if (typeof callback == 'function')
                callback.call(params_in.root, pageInto, pageOut, options);
    ...
    Mobilebone.createPage = function (domHtml, eleOrObj, options) {
        ...
        if (Mobilebone.evalScript == true && domHtml.firstintoBind != true) {
            slice.call(create.getElementsByTagName('script')).forEach(function (originScript) {
                ...
                if (scriptContent.trim() == '' || originScript.src)
                    return;
        ...
        } else if (create_title && create_title.innerText) {
    ...
    Mobilebone.ajax = function (aOrFormOrObj) {
        ...
        if (aOrFormOrObj.nodeType == 1) {
            ...
            if (tagName == 'form') {
                ...
                var idContainer = aOrFormOrObj.getAttribute('data-container'), classPageInside = aOrFormOrObj.getAttribute('data-classpage'), container = idContainer && document.getElementById(idContainer);
	...
	**/
    Mobilebone.submit = function (form) {
        ...
        if (ajax == 'false' || Mobilebone.captureForm == false && ajax != 'true')
            return;
	...
	**/
    Mobilebone.init = function () {
        if (hasInited == true)
            return 'Don\'t repeat initialization!';
        var hash = location.hash.replace('#&', '#'), ele_in = null;
        if (hash == '' || hash == '#') {
            this.transition(document.querySelector('.' + this.classPage));
        } else if (isSimple.test(hash) == true && (ele_in = document.querySelector(hash)) && ele_in.classList.contains(this.classPage)) {
            // 'ele_in' must be a page element
	...
	**/
    Mobilebone.handleTapEvent = function (event) {
		...
		*/
        ...
        var idContainer = target.getAttribute('data-container'), classPageInside = target.getAttribute('data-classpage'), container = idContainer && document.getElementById(idContainer);
        if (container && classPageInside && classPageInside != Mobilebone.classPage) {
	...
	**/
    window.addEventListener('popstate', function () {
        ...
        if (page_in && page_in == page_out || Mobilebone.pushStateEnabled == false)
            return;
        // hash ↔ id													
        if (page_in) {
